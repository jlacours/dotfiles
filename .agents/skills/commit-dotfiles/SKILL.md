...
8. Run validation proportional to the changed files. Prefer repository-native checks, then use syntax tools such as `bash -n`, `zsh -n`, `qmllint`, `cargo test`, or application-specific config checks. Never auto-format unrelated files.
   - When game mode or its Hyprland/Quickshell integration changed, validate the complete control path: run `bash -n` on `game-mode.sh`, confirm every `MANAGED_UNITS` entry resolves with `systemctl --user show`, run `game-mode.sh status`, check the `Super+Alt+G` binding, run `qmllint` on the game-mode state/chip and bar, and inspect the live Quickshell log after reload.
   - Exercise one real `on` → `off` cycle when a graphical Hyprland session is available. Record which units were active first, prove only those units restart, confirm Mako DND and the CPU governor restore to their prior values, and leave game mode off. Do not stop the current agent's control channel if it depends on a managed unit; defer that unit and report the limitation.
