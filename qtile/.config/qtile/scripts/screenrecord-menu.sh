...
# Screen recording Fuzzel wrapper script
# A multi-level menu interface for wf-recorder, Qtile only.

# Configuration
RECORDINGS_DIR="$HOME/Videos/recordings"
STATE_DIR="${XDG_RUNTIME_DIR:-/tmp}/qtile-screenrecord-$UID"
...
# "X,Y WxH" for wf-recorder's -g. Empty when Qtile cannot be queried.
focused_screen_geometry() {
...
# Geometry of the currently focused Qtile window, in layout coordinates, as
# "X,Y WxH" for wf-recorder's -g. Empty when Qtile cannot be queried.
focused_window_geometry() {
    qtile cmd-obj -o window -f info 2>/dev/null | python3 -c '
import ast, sys
try:
    d = ast.literal_eval(sys.stdin.read())
    print("{},{} {}x{}".format(d["x"], d["y"], d["width"], d["height"]))
...
# Function to show the Fuzzel menu overlay and get selection
# $1 = prompt text
# $2 = options (newline-separated string)
show_menu() {
    local prompt=$1
    local options=$2
    # Fuzzel is always case-insensitive.
    # --only-match: only allow selecting from list, no custom input
    printf '%b\n' "$options" | fuzzel --dmenu --only-match --minimal-lines --prompt "$prompt> "
}

# Function to select recording area
#
# Qtile has no per-output name lookup like Hyprland's, so both "Active
# output" and "Active window" resolve to a geometry string (via Qtile's own
# introspection) and are always passed to wf-recorder with -g, never -o.
select_area() {
    local area=$1
    local geometry=""
    case $area in
        "Active output")
            geometry=$(focused_screen_geometry)
            if [ -n "$geometry" ]; then
                echo "$geometry"
            else
                echo "CANCEL"
            ...
            ;;
        "Active window")
            geometry=$(focused_window_geometry)
            if [ -n "$geometry" ]; then
                echo "$geometry"
            else
                echo "CANCEL"
...
start_recording() {
    ...
    # Qtile has no per-output name lookup, so any geometry we were handed
    # ("x,y WxH", whether it's the focused screen, the focused window, or a
    # slurp-selected region) is always passed via -g, never -o.
    if [ -n "$geometry" ]; then
        cmd_args+=(-g "$geometry")
    fi
    ...
    # Capture diagnostics instead of leaking stderr into the menu. Do not
...
# Main menu flow
main() {
    local -a required_commands=(notify-send pactl python3 qtile slurp wf-recorder)
    ...
    if [ "$geometry" = "CANCEL" ]; then
        # Covers both a user-cancelled slurp selection and a failed/empty
        # Qtile geometry lookup (no window, no screen info, qtile not
        # reachable, etc.) -- either way there is nothing to record.
        notify-send -a "Screen Recorder" -u low "Recording cancelled" "No geometry was found for the selected area"
        exit 0
    fi
